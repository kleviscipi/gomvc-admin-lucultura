    ...
    $.fn.extend({
        smartpaginator: function (options) {
            ...
            return this.each(function () {
                ...
                var list = $('<ul/>');
                var btnPrev = $('<div/>').text(settings.prev).live('click',function () { if ($(this).hasClass('disabled')) return false; currentPage = parseInt(list.find('li a.active').text()) - 1; navigate(--currentPage); }).addClass('btn');
                    ...
                    if (topage >= 0 && topage < totalpages) {
                        ...
                        if (dataContainer != null) {
                            if (dataContainer.length > 0) {
                                ...
                                if ($(dataElements[0]).find('live','th').length > 0) { //if there is a header, keep it visible always
